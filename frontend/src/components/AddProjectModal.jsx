...
export default function AddProjectModal({ onClose, onSuccess }) {
    ...
    // 2. Watch for 100% completion to trigger redirect
    useEffect(() => {
        if (progressPercent === 100 && extractedRepoName) {
            // Wait a brief moment so the user can read "Complete!" before the modal vanishes
            const timer = setTimeout(() => {
                onSuccess(extractedRepoName);
            }, 800);
            return () => clearTimeout(timer);
        }
    }, [progressPercent, extractedRepoName, onSuccess]);
    ...
    const handleSubmit = async (e) => {
        ...
        // Extract repo name: "https://github.com/user/react.git" -> "react"
        ...
        try {
            ...
            if (!res.ok) throw new Error((await res.json()).detail || 'Failed to start cloning project');
            
            // NOTE: We do not call onSuccess here! 
            // The backend returns instantly (Background Task), so we wait for the WebSocket to hit 100%.
    ...
    return (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, background: 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <div style={{ background: '#fff', padding: '30px', borderRadius: '16px', width: '400px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                <h2 style={{ margin: '0 0 20px 0', color: '#0f172a' }}>Import from GitHub</h2>
                ...
                {!isLoading ? (
                    <form onSubmit={handleSubmit}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                            ...
                            <button type="submit" style={{ padding: '10px 16px', borderRadius: '8px', border: 'none', background: '#2563eb', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>Clone & Parse</button>
                        </div>
                    </form>
                ) : (
                    <div style={{ textAlign: 'center', padding: '20px 0' }}>
                        <div style={{ fontSize: '14px', fontWeight: 600, color: '#0f172a', marginBottom: '8px' }}>{progressStatus || 'Initializing...'}</div>
